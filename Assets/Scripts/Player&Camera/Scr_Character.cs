...
public class Scr_Character : MonoBehaviour {
	...
	void Start()
	{
		...
		Layers = GetComponent<Scr_AnimationLayers>();
	}
	...
	public void SetSwim(bool swim){
		...
		if(swim){
			...
			//animator, deckkraft, (layer nummer 1, deckkraft) via coroutine
			Layers.Weights[1] = 1f;
		}
		else{
			Vector3 trueRotation = new Vector3( 0, transform.rotation.eulerAngles.y, 0);
			transform.rotation = Quaternion.Euler(trueRotation);
			Layers.Weights[1] = 0f;
		}
	}
